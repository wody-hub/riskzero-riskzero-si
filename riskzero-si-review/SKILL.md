...
이 스킬은 riskzero-si 8단계 파이프라인의 **Step 4**이다. 리뷰 결과는 콘솔 출력에 더해 **`.si-planning/{기능명}/code-review.md`로도 저장**한다(다음 단계가 `/clear` 후에도 결과를 읽을 수 있도록 자기완결적 산출물을 남긴다). 그런 다음 답변 맨 끝에 아래 푸터를 반드시 출력한다. (`{기능명}`은 실제 값으로 채운다.)
...
> |---|------|------|------|
> | 1~3 | 계획·리뷰·구현 | … | ✅ |
> | 4 | 표준 리뷰 | `riskzero-si-review` | 👉 방금 완료 |
> | 5 | PR 리뷰 | `riskzero-si-pr-review` | 👉 다음 |
> | 6 | QA 체크리스트 | `riskzero-si-qa-checklist` | ⬜ |
> | 7 | 버그 수정 | `riskzero-si-qa` | ⬜ |
> | 8 | 최종 검증 | `riskzero-si-browse` | ⬜ |
>
> **방금 완료:** Step 4 — 산출물: `.si-planning/{기능명}/code-review.md` (판정: PASS/CONDITIONAL PASS/FAIL)
>
> **▶️ 다음 단계 — 아래 명령어를 복사해 실행하세요:**
> ```
> /riskzero-si-pr-review {기능명}
> ```
> *(FAIL이면 먼저 ERROR 항목의 코드를 수정한 뒤 `/riskzero-si-review {기능명}`으로 재리뷰하세요.)*
>
