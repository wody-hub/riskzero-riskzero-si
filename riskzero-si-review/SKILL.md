...
### 진행 게이트 (Progression Gate) — 공통 원칙

다음 단계로 진행할지는 **판정 라벨(PASS/CONDITIONAL PASS/FAIL)이 아니라 발견사항의 심각도로 결정한다.** (PASS 라벨도 Medium을 포함할 수 있으므로 라벨만으로 통과 판단 금지.)

- **차단 등급 = Critical / High / Medium**, 그리고 영역별 FAIL.
- **차단 발견사항이 0건일 때만** 다음 단계(Step 5: PR 리뷰)로 진행한다.
- **1건이라도 있으면**: 다음 단계로 넘어가지 말고 → 해당 항목을 조치(코드 수정) → `/riskzero-si-review {기능명}`로 **재리뷰** → 차단 0건(clean)이 될 때까지 반복한 뒤 진행한다.
- **Low / 정보성**은 비차단(권고). 푸터에 "권장 조치"로 남기되 진행을 막지 않는다.
- **예외(명시적 수용)**: 사용자가 특정 차단 항목을 명시적으로 수용/보류 결정한 경우(예: 별도 과제로 분리), 산출물에 **사유와 후속 추적 위치**를 기록하면 그 항목은 차단에서 제외할 수 있다. 임의 무시는 금지한다.
...
이 스킬은 riskzero-si 8단계 파이프라인의 **Step 4**이다. 리뷰 결과는 콘솔 출력에 더해 **`.si-planning/{기능명}/code-review.md`로도 저장**한다(다음 단계가 `/clear` 후에도 결과를 읽을 수 있도록 자기완결적 산출물을 남긴다). 그런 다음 **§10 진행 게이트로 차단 발견사항(Medium 이상, 영역 FAIL) 건수를 집계**하고, 그 결과에 따라 아래 두 푸터 중 **하나만** 답변 맨 끝에 출력한다. (`{기능명}`은 실제 값으로 채운다.)
...
> |---|------|------|------|
> | 1~3 | 계획·리뷰·구현 | … | ✅ |
> | 4 | 표준 리뷰 | `riskzero-si-review` | 👉 방금 완료 (통과) |
> | 5 | PR 리뷰 | `riskzero-si-pr-review` | 👉 다음 |
> | 6 | QA 체크리스트 | `riskzero-si-qa-checklist` | ⬜ |
> | 7 | 버그 수정 | `riskzero-si-qa` | ⬜ |
> | 8 | 최종 검증 | `riskzero-si-browse` | ⬜ |
>
> **방금 완료:** Step 4 — 산출물: `.si-planning/{기능명}/code-review.md` (차단 0건, 잔여 Low/권고만)
>
...
> ```
>
> 💡 컨텍스트가 길어졌다면 `/clear` 후 위 명령어를 그대로 붙여넣으세요. 다음 단계는 git diff와 `.si-planning/{기능명}/` 산출물을 읽어 단독 실행됩니다.
> ---

**(B) 차단 발견사항 ≥1건 — 조치 필요 시 (다음 단계로 진행하지 않는다):**
...
> |---|------|------|------|
> | 1~3 | 계획·리뷰·구현 | … | ✅ |
> | 4 | 표준 리뷰 | `riskzero-si-review` | 🔴 조치 필요 (차단 {N}건) |
> | 5 | PR 리뷰 | `riskzero-si-pr-review` | ⛔ 대기 |
>
> **방금 완료:** Step 4 — 산출물: `.si-planning/{기능명}/code-review.md` (차단 {N}건: Critical {c} / High {h} / Medium {m})
>
> **⚠️ 다음 단계로 진행하기 전에 차단 항목을 먼저 조치해야 합니다.** 위 리뷰의 Critical/High/Medium 항목을 코드로 수정한 뒤, 아래 명령으로 **재리뷰**하세요. 차단 0건이 될 때까지 반복합니다.
> ```
> /riskzero-si-review {기능명}
> ```
> *(특정 차단 항목을 의도적으로 수용/보류한다면, code-review.md에 사유와 후속 추적 위치를 기록한 항목에 한해 차단에서 제외할 수 있습니다.)*
>
