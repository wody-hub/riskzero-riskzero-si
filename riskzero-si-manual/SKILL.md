...
- **페이지탭 누적**: 이 앱은 탭 시스템이라 한 페이지에서 연속 이동하면 비가시 탭 DOM이
  locator에 걸린다 → 러너가 블록마다 새 페이지를 생성해 격리함. 한 블록 안의 상태 조작은
  반드시 `prepare` 안에서 해결할 것 (블록 간 상태 공유 불가)
- **숨은 인쇄 템플릿**: 상세 화면 라벨이 숨겨진 인쇄용 DOM에도 존재 → 러너의 visibleBox가
  가시 요소까지 순회하므로 스펙에서는 `.first()` 걱정 없이 라벨 텍스트로 지정하면 됨
- **긴 상세/등록 화면**: 기본 viewport(1920×900)에 안 잡히면 `capture.viewport`로 키울 것
  (예: `{ width: 1920, height: 1750 }`) — clip은 viewport 안에서만 유효
- **세션 단일화**: 다른 곳에서 같은 계정 로그인 시 세션이 끊김 → 러너가 자동 재로그인하지만,
  배치 중에는 같은 계정으로 웹 접속하지 말 것
