...
모든 스킬은 한 단계를 끝낼 때마다 답변 맨 끝에 **단계 종료 푸터**를 출력한다 — ① 전체 8단계 중 현재 위치, ② 복사해서 바로 실행할 다음 단계 명령어(기능명이 채워진 완전한 형태), ③ `/clear` 안내.

이 덕분에 단계 사이마다 **컨텍스트를 비우고(`/clear`) 다음 단계만 새로 띄우는 운용**이 가능하다:

1. 한 단계가 끝나면 푸터의 "다음 단계 명령어"를 복사한다
2. `/clear`로 대화 컨텍스트를 비운다
3. 복사한 명령어를 붙여넣는다 — 다음 스킬이 `.si-planning/{기능명}/` 산출물을 읽어 단독 실행된다

각 단계 산출물이 자기완결적으로 디스크에 남기 때문에 가능한 흐름이다. 긴 기능을 8단계 내내 한 컨텍스트로 끌고 가면 토큰이 누적되어 후반 단계 품질이 떨어지므로, **단계마다 `/clear`하는 것을 권장한다.**

> ⚠️ 다음 명령어에는 항상 기능명이 채워져 출력된다. `/clear` 후에는 기능명을 추론할 수 없으므로 반드시 푸터의 완전한 명령어를 쓴다. 단 `/riskzero-si-qa-checklist`만 기능명이 아니라 **메뉴명/URL/화면ID**를 받는다(Step 5 푸터가 안내).
...
| 버전 | 날짜 | 기준 커밋 | 검증일 | 내용 |
|---|---|---|---|---|
| **v0.3** | 2026-06-18 | `riskzero-riskzero-si@1b7df34` | 2026-06-18 | 단계 종료 푸터 규약 반영 — 모든 스킬이 단계 종료 시 전체 진행 현황 + 복붙 가능한 다음 단계 명령어(기능명 포함) + `/clear` 안내를 출력. 18장에 푸터/`/clear` 운용 추가, README에 동일 안내 추가 |
