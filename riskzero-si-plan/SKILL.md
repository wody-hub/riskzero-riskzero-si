...
2. 없으면 프로젝트 루트까지 상위 탐색
3. 없으면 ~/.claude/ 하위 탐색
4. 끝까지 없으면 사용자에게 경로를 질문한다
```
...
```
{config.output.planDir}/impl-{기능명}.md
```

`config.output.planDir`이 설정되지 않은 경우:
1. 프로젝트 루트의 `plan/` 디렉토리 사용
2. 디렉토리가 없으면 생성
...
| `output.planDir` | 프로젝트 루트의 `plan/` 사용 |
