...
OUTPUT_DIR   = expand(config.outputs.root, config.outputs.featureDirPattern, 기능명)
SCREENSHOT_DIR = OUTPUT_DIR/config.outputs.evidenceDir/config.outputs.screenshotDir
LOG_DIR = OUTPUT_DIR/config.outputs.evidenceDir/config.outputs.logsDir
```
...
# 5) 증거 스크린샷
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-{테스트ID}.png
...
$B wait --networkidle
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-A-2-1.png
# 에러 메시지 확인
...
$B click "button:has-text('저장')"
$B wait --networkidle
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-A-2-6.png
# 목록으로 이동 확인
$B url  # 목록 URL 포함 확인
```
...
$B wait --networkidle
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-B-1.png
# 입력 필드 editable 확인
...
$B wait --networkidle
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-C-1-1.png
# 결과 건수 확인
...
$B wait --networkidle
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-C-3-1.png
# 필터 초기값 확인
...
$B wait --networkidle
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-D-1.png
...
$B wait --networkidle
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-E-2.png
# 검색조건 유지 확인
$B js "document.querySelector('input[name=\"searchText\"]')?.value"
# "검색어"이면 PASS
```
...
$B wait 500
$B screenshot $SCREENSHOT_DIR/qa-{도메인}-F-1.png
# 다이얼로그 확인
...
- `screenshot` 경로는 항상 `$SCREENSHOT_DIR` 하위에 생성
...
| 파일명 | 테스트 ID | 설명 |
|--------|----------|------|
| evidence/screenshots/qa-{도메인}-A-2-1.png | A-2-1 | 빈 폼 저장 결과 |
```
