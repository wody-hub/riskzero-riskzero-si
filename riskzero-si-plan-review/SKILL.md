...
## 종합 판정
- PASS: 차단 항목 0건 → 진행 가능
- WARN: FAIL 항목은 없고 Low/권고만 있음 → 진행 가능(권고 표기)
- FAIL: 차단 항목(아래) 존재 → 계획 수정 후 재리뷰
```

### 6. 진행 게이트 (Progression Gate)

다음 단계 진행 여부는 **체크 항목의 FAIL 여부로 결정**한다(판정 라벨이 아니라 발견사항 기준).

- **차단 = 체크 항목 중 FAIL 1건 이상, 또는 gstack 리뷰의 수정필요(Critical/High/Medium 등) 지적.**
- **차단 0건(전 항목 PASS, 잔여는 Low/권고만)일 때만** 다음 단계(Step 3: 구현)로 진행한다.
- **1건이라도 있으면**: 구현으로 넘어가지 말고 → `/riskzero-si-plan {기능명}`으로 계획을 수정 → `/riskzero-si-plan-review {기능명}`로 **재리뷰** → 차단 0건이 될 때까지 반복한 뒤 진행한다.
- **Low/권고**는 비차단. 다만 plan-review.md에 남겨 구현 단계에서 참고하게 한다.
- **예외(명시적 수용)**: 사용자가 특정 차단 항목을 명시적으로 수용/보류한 경우, plan-review.md에 사유·후속 추적 위치를 기록한 항목에 한해 차단에서 제외할 수 있다.

---

## 단계 종료 시 필수 출력 (단계 종료 푸터)

이 스킬은 riskzero-si 8단계 파이프라인의 **Step 2**이다. 리뷰 산출물 저장 후 **§6 진행 게이트로 차단 항목(FAIL/수정필요) 건수를 집계**하고, 그 결과에 따라 아래 두 푸터 중 **하나만** 답변 맨 끝에 출력한다. (`{기능명}`은 실제 값으로 채운다.)
...
> | 2 | 계획 리뷰 | `riskzero-si-plan-review` | 👉 방금 완료 (통과) |
> | 3 | 구현(TDD) | `riskzero-si-impl` | 👉 다음 |
> | 4~8 | 리뷰·QA·검증 | … | ⬜ |
>
> **방금 완료:** Step 2 — 산출물: `.si-planning/{기능명}/plan-review.md` (차단 0건, 잔여 Low/권고만)
>
> **▶️ 다음 단계 — 아래 명령어를 복사해 실행하세요:**
> ```
> /riskzero-si-impl {기능명}
> ```
>
> 💡 컨텍스트가 길어졌다면 `/clear` 후 위 명령어를 그대로 붙여넣으세요. 다음 단계는 `.si-planning/{기능명}/` 산출물을 읽어 단독 실행됩니다.
> ---

**(B) 차단 항목 ≥1건 — 조치 필요 시 (구현으로 진행하지 않는다):**
...
> |---|------|------|------|
> | 1 | 논의+계획 | `riskzero-si-plan` | ✅ |
> | 2 | 계획 리뷰 | `riskzero-si-plan-review` | 🔴 조치 필요 (차단 {N}건) |
> | 3 | 구현(TDD) | `riskzero-si-impl` | ⛔ 대기 |
>
> **방금 완료:** Step 2 — 산출물: `.si-planning/{기능명}/plan-review.md` (차단 {N}건)
>
> **⚠️ 구현으로 진행하기 전에 계획의 차단 항목을 먼저 조치해야 합니다.** 아래 명령으로 계획을 수정한 뒤 재리뷰하세요. 차단 0건이 될 때까지 반복합니다.
> ```
...
> ```
> *(특정 차단 항목을 의도적으로 수용/보류한다면, plan-review.md에 사유·후속 추적 위치를 기록한 항목에 한해 차단에서 제외할 수 있습니다.)*
>
