...
이 스킬은 riskzero-si 8단계 파이프라인의 **Step 2**이다. 리뷰 산출물 저장 후 답변 맨 끝에 아래 푸터를 반드시 출력한다. (`{기능명}`은 실제 값으로 채운다. 게이트 결과에 따라 "다음 단계" 명령어를 선택한다.)
...
> | 2 | 계획 리뷰 | `riskzero-si-plan-review` | 👉 방금 완료 |
> | 3 | 구현(TDD) | `riskzero-si-impl` | 👉 다음 |
> | 4~8 | 리뷰·QA·검증 | … | ⬜ |
>
> **방금 완료:** Step 2 — 산출물: `.si-planning/{기능명}/plan-review.md` (판정: PASS/WARN/FAIL)
>
> **▶️ 다음 단계 — 아래 명령어를 복사해 실행하세요:**
> ```
> /riskzero-si-impl {기능명}
> ```
> *(FAIL/CRITICAL이면 대신 `/riskzero-si-plan {기능명}`으로 계획을 수정하세요.)*
>
