...
### 3.4. 파일 충돌 사전 확인 (구현 시작 전 1회)

구현을 시작하기 전, 계획서의 파일 배치 목록을 기준으로 생성/수정 대상 전체를 한 번에 확인한다:

1. 계획서에서 생성·수정 예정 파일 경로를 전부 추출한다.
2. Glob으로 각 경로의 **기존 존재 여부**를 확인하여 두 그룹으로 나눈다:
   - 신규 생성 (기존 파일 없음)
   - **기존 파일 수정/덮어쓰기** (라우터 등록 등 계획서에 명시된 수정은 제외)
3. 덮어쓰기 그룹이 비어 있으면 확인 없이 바로 진행한다.
4. 덮어쓰기 그룹이 있으면 목록을 출력하고 AskUserQuestion으로 **일괄 1회** 확인을 받는다.
   파일별로 반복 질문하지 않는다 — 자동 파이프라인 흐름을 유지하기 위함이다.
5. 사용자가 일부 파일을 제외하면, 해당 파일은 건드리지 않고 구현 결과 보고서의 "참고 사항"에 기록한다.
...
이 스킬은 riskzero-si 8단계 파이프라인의 **Step 3**이다. 위 "구현 결과"에 이어 답변 맨 끝에 아래 푸터를 반드시 출력한다. (`{기능명}`은 실제 값으로 채운다.)
...
>
> **방금 완료:** Step 3 — 산출물: 생성/수정된 소스 파일 + `.si-planning/{기능명}/tdd-report.md`
>
> **▶️ 다음 단계 — 아래 명령어를 복사해 실행하세요:**
> ```
> /riskzero-si-review {기능명}
> ```
> *(빌드/린트가 실패한 상태면 먼저 오류를 수정한 뒤 다시 진행하세요.)*
>
