...
프론트엔드 production code 작성 전에 `tdd-plan.md`의 FE 테스트를 먼저 작성한다.

- 사용자에게 보이는 동작은 Testing Library/Vitest/Jest/Cypress 등 프로젝트 표준 테스트 도구로 검증한다.
- API 훅은 성공/실패/로딩 상태를 최소 하나 이상 검증한다.
- 폼은 필수값, 정상 저장, 서버 에러 표시 중 계획서에 지정된 핵심 동작을 먼저 검증한다.
...
### 8.3. 테스트 검증
```bash
# config.frontend.testCmd 실행
npm test  # 또는 npm run test:unit, npx vitest run
```
- RED에서 작성한 테스트가 GREEN으로 전환됐는지 확인
- 테스트 결과 로그를 `evidence/test-results/` 아래에 저장하거나 `tdd-report.md`에 요약
