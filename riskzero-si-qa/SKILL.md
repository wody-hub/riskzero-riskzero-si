...
이 스킬은 riskzero-si 8단계 파이프라인의 **Step 7**이다. 모든 FAIL 항목이 PASS되면 답변 맨 끝에 아래 푸터를 반드시 출력한다. (`{기능명}`은 실제 값으로 채운다.)
...
> |---|------|------|------|
> | 1~6 | 계획~QA체크리스트 | … | ✅ |
> | 7 | 버그 수정 | `riskzero-si-qa` | 👉 방금 완료 |
> | 8 | 최종 검증 | `riskzero-si-browse` | 👉 다음 |
>
> **방금 완료:** Step 7 — 산출물: `.si-planning/{기능명}/qa-report.md` + 수정된 소스 파일 (체크리스트 전 항목 PASS)
>
> **▶️ 다음 단계 — 아래 명령어를 복사해 실행하세요:**
> ```
> /riskzero-si-browse {기능명 또는 URL}
> ```
> *(5회 이상 미해결 버그가 남아 있으면 다음으로 넘어가지 말고 사용자에게 보고하세요.)*
>
> 💡 컨텍스트가 길어졌다면 `/clear` 후 위 명령어를 그대로 붙여넣으세요. 다음 단계는 `.si-planning/{기능명}/qa-checklist.md`를 읽어 단독 실행됩니다.
> ---
